...
class DoodleVideoGeneratorV7_5:
    """
    V7.5: V7.4 Dark Base + Imagen4.
    """
    ...
    def __init__(self, image_path, output_path, segments=None, duration=5.0, fps=24, style='normal'):
        ...
        self.fps = fps
        self.style = style # 'normal', 'solid', 'pencil', 'dark'
        self.width = 1920
        self.height = 1080
    ...
    def _get_cleaned_image(self, img_bgr):
        ...
        # Auto-detect, or force if style is dark
        self.is_dark_bg = (mean_brightness < 127) or (self.style == 'dark')
        ...
        # Shared kernel for separating text
        text_sep_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        
        if self.style == 'dark':
            # Dark Mode: Global Threshold to capture ALL light pixels (Neon/Chalk)
            # Increased constraint (30->60) to avoid "blooming" connecting letters
            _, binary_out = cv2.threshold(gray, 60, 255, cv2.THRESH_BINARY)
            
            # AGGRESSIVE SEPARATION: Erode then Dilate to break links
            # Using 2x2 for erosion to nip thin connections
            erosion_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
            binary_out = cv2.erode(binary_out, erosion_kernel, iterations=1)
            binary_out = cv2.dilate(binary_out, erosion_kernel, iterations=1)
            
        elif self.style == 'solid':
            # Solid/Infographic: Global Threshold to capture ALL non-white pixels.
            _, binary_out = cv2.threshold(gray, 250, 255, cv2.THRESH_BINARY_INV)
            
            # AGGRESSIVE SEPARATION for Solid Text
            # Solid text is thicker, so we can erode more safely
            erosion_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
            binary_out = cv2.erode(binary_out, erosion_kernel, iterations=1)
            binary_out = cv2.dilate(binary_out, erosion_kernel, iterations=1)
            
        else: # 'normal', 'pencil'
            # Normal/Pencil: Adaptive Thresholding
            binary_sketch = cv2.adaptiveThreshold(
                gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, thresh_type, 11, 2
            )
            # Clean tiny noise
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
            binary_out = cv2.morphologyEx(binary_sketch, cv2.MORPH_OPEN, kernel)
            
        # 3. MASK BORDER (Fix for horizontal line artifact at top)
        # Explicitly zero out the 5px border to prevent framing artifacts
        h_b, w_b = binary_out.shape
        binary_out[0:5, :] = 0  # Top
        binary_out[-5:, :] = 0  # Bottom
        binary_out[:, 0:5] = 0  # Left
        binary_out[:, -5:] = 0  # Right
    ...
    def _sort_contours_by_path(self, glyphs):
        ...
        Replaces NearestNeighbor to fix the 'Zig-Zag' (Snake) effect where 
        pen draws Line 1 L->R, then Line 2 R->L.
        """
        if not glyphs: return []
        
        # 1. Sort primarily by Y to handle vertical flow
        # But simple Y sort is not enough for lines (items slightly offset in Y).
        # We need to bin them into "visual lines".
        
        # Initial sort by Y
        y_sorted = sorted(glyphs, key=lambda g: g['y'])
        
        lines = []
        if not y_sorted: return []
        
        current_line = [y_sorted[0]]
        # Track line stats
        # We compare against the LINE AVERAGE Center-Y, not just the previous neighbor.
        # This prevents "drift" where a slightly lower letter drags the threshold down/up
        # or a single small punctuation breaks the chain.
        
        for g in y_sorted[1:]:
            cy = g['center'][1]
            
            # Calculate current line average Y and H
            line_avg_cy = sum(item['center'][1] for item in current_line) / len(current_line)
            line_avg_h = sum(item['h'] for item in current_line) / len(current_line)
            
            # Compare current glyph to the LINE AVERAGE
            avg_h = (g['h'] + line_avg_h) / 2
            
            if abs(cy - line_avg_cy) < (avg_h * 0.5):
                current_line.append(g)
            else:
                # New Line: Sort previous line by X
    ...
    def _safely_merge_words(self, glyphs):
        ...
        Strict thresholds prevent merging diagrams. (Exact V7.3 logic)
        """
        if not glyphs: return []
        
        # 1. Separate "Likely Text" (Small Area) from "Likely Diagram" (Large Area)
        # Threshold: Area < 30000 (Increased for Solid/Bold text)
                            ...
                            for c in active_glyph['contours']:
                                ...
                                if current_len + c_len <= target_glyph_len:
                                    ...
                                    if needed > 0 and c_len > 0:
                                        ...
                                        if pts_to_draw > 0:
                                            # Using thickness=12 to mimic marker tip for the partial draw
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        gen = DoodleVideoGeneratorV7_5(sys.argv[1], sys.argv[2], duration=dur, style=style)
        gen.generate()
