#!/usr/bin/env python3
"""
Generate Topic Video V7.2 (Skeleton Doodle + Sync)
1. Generate Beat Manifest with PARTS.
2. Generate Image (Fal.ai).
3. Generate Audio per-part.
4. Generate Video using DoodleVideoGeneratorV7_2 (Skeletonization).
"""
...
from create_doodle_video_v7_2 import DoodleVideoGeneratorV7_2
...
def log(msg):
    print(f"[TopicVideoV7.2] {msg}", flush=True)
...
def generate_beat_manifest(topic):
    log(f"🧠 Generating V7.2 Manifest (with parts) for: {topic}...")
...
def main():
    ...
    parser.add_argument("--output_dir", default="topic_videos_v7_2", help="Output dir")
    ...
    for i, beat in enumerate(beats):
        ...
        # E. Generate Doodle (V7.2)
        if not os.path.exists(final_beat_path):
            try:
                gen = DoodleVideoGeneratorV7_2(
                    image_path, 
                    final_beat_path, 
                    segments=segments,
    ...
    if final_clips:
        log("\n🎞️ Stitching...")
        out_path = os.path.join(work_dir, f"video_v7_2_{safe_topic}.mp4")
