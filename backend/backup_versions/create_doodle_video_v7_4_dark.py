...
class DoodleVideoGeneratorV7_4_Dark:
    """
    V7.4 Dark: V7.4 Base + Dark Theme Support.
    """
    ...
    def _get_cleaned_image(self, img_bgr):
        ...
        if self.style == 'dark':
            ...
            # Avoids "hollowing" caused by adaptive.
            _, binary = cv2.threshold(gray, 30, 255, cv2.THRESH_BINARY)
            return binary
            
        elif self.style == 'solid':
            # Solid/Infographic: Global Threshold to capture ALL non-white pixels.
            # This ensures solid blocks (banners) are FILLED, not just outlined.
            # Using 250 allows very light colors to be detected.
            _, binary = cv2.threshold(gray, 250, 255, cv2.THRESH_BINARY_INV)
            return binary
            
        else: # 'normal', 'pencil'
            # Normal/Pencil: Adaptive Thresholding (Preserves texture/sketchiness/lines)
    ...
    def _sort_contours_by_path(self, glyphs):
        ...
        current_h = y_sorted[0]['h']
        
        for g in y_sorted[1:]:
            # Overlap criteria for "Same Line":
            # If g['y'] overlaps significantly with current line's vertical span
            # Simple heuristic: if g['y'] is within center-band of previous
            # or if vertical distance is small.
            
            # Using Center Y comparison is usually best.
            cy = g['center'][1]
            prev_cy = current_line[-1]['center'][1]
            avg_h = (g['h'] + current_line[-1]['h']) / 2
            
            if abs(cy - prev_cy) < (avg_h * 0.5):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        gen = DoodleVideoGeneratorV7_4_Dark(sys.argv[1], sys.argv[2], duration=dur, style=style)
        gen.generate()
