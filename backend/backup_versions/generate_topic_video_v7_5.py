...
from create_doodle_video_v7_5 import DoodleVideoGeneratorV7_5
...
def log(msg):
    print(f"[TopicVideoV7.5] {msg}", flush=True)
...
def generate_image_fal(prompt, output_path, style="normal"):
    ...
    # Clean the prompt to remove "whiteboard" triggers which cause physical frames
    clean_prompt = prompt.replace("whiteboard", "diagram").replace("Whiteboard", "Diagram").replace("blackboard", "background").strip()
    ...
    if style == "pencil":
        ...
        negative_prompt = "frame, border, whiteboard frame, monitor, screen, bezel, color, ink, marker, heavy lines, solid black, photo, realistic, 3d, digital art"
        ...
        negative_prompt = "frame, border, whiteboard frame, monitor, screen, bezel, white background, light mode, inverted, color, complex, filled, gradients, photo, realistic"
        ...
        negative_prompt = "frame, border, whiteboard frame, monitor, screen, bezel, color, colored, complex, filled, gradients, photo, realistic, dark background, blue background, blurry, vertical lines, divider lines, section borders, grid lines"
        
    else: # solid / color (Now "Infographic")
        # Removing "infographic" as it can sometimes trigger posters/frames. Using "digital vector illustration".
        style_suffix = ", digital vector illustration on pure white background, distinct isolated elements, colored pencil style, educational, clean lines, high resolution, 8k, no background texture, no borders"
        negative_prompt = "whiteboard, frame, border, monitor, screen, bezel, plastic edge, easel, furniture, room, wall, grayscale, black and white, monochrome, dark background, texture, heavy fills, painting, realistic, photo, 3d, gradient, blurry, messy, sketch, pencil"
