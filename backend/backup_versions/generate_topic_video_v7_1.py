#!/usr/bin/env python3
"""
Generate Topic Video V7.1 (Segment-Synchronized)
1. Generate Beat Manifest with PARTS (position + audio_script).
2. Generate Image (Fal.ai).
3. Generate Audio per-part (ElevenLabs).
4. Generate Video using DoodleVideoGeneratorV7_1 with segments.
5. Mux & Stitch.
"""
...
from create_doodle_video_v7_1 import DoodleVideoGeneratorV7_1
...
def log(msg):
    print(f"[TopicVideoV7.1] {msg}", flush=True)
...
def generate_beat_manifest(topic):
    log(f"🧠 Generating V7.1 Manifest (with parts) for: {topic}...")
    ...
    ### EXAMPLE FOR "Linear vs Logistic Regression"
    {{
      "topic": "Linear vs Logistic Regression",
      "beats": [
        {{
          "beat_id": 1,
          "image_prompt": "Whiteboard diagram showing two side-by-side comparisons: LEFT shows linear regression with scattered points and a straight line, RIGHT shows logistic regression with an S-curve separating classes. NO visible divider lines.",
          "parts": [
            {{"position": 0, "visual_desc": "Linear regression scatter plot with best-fit line", "audio_script": "Linear regression predicts continuous values. The model fits a straight line through the data to minimize errors."}},
            {{"position": 2, "visual_desc": "Logistic regression S-curve with class separation", "audio_script": "Logistic regression predicts probabilities for classification. It uses a sigmoid curve to separate classes."}}
          ]
        }}
      ]
    }}
...
def main():
    ...
    parser.add_argument("--output_dir", default="topic_videos_v7_1", help="Output dir")
    ...
    for i, beat in enumerate(beats):
        ...
        # C. Generate audio for each part and collect segments
        audio_clips = []
        ...
        for p_idx, part in enumerate(parts):
            ...
            try:
                ...
                log(f"   🎙️ Part {p_idx} (pos={p_position}): {ac.duration:.1f}s - {p_script[:35]}...")
            except Exception as e:
        ...
        total_duration = full_audio.duration + 1.0  # Buffer
        
        log(f"   📊 {len(segments)} segments, total duration: {total_duration:.1f}s")
        
        # E. Generate Doodle Video with segment info
        if not os.path.exists(final_beat_path):
            try:
                gen = DoodleVideoGeneratorV7_1(
                    image_path, 
                    final_beat_path, 
                    segments=segments,
    ...
    if final_clips:
        log("\n🎞️ Stitching...")
        out_path = os.path.join(work_dir, f"video_v7_1_{safe_topic}.mp4")
