#!/usr/bin/env python3
"""
Generate Topic Video V7.3 (Hierarchy Sort Doodle + Sync)
1. Generate Beat Manifest with PARTS.
2. Generate Image (Fal.ai).
3. Generate Audio per-part.
4. Generate Video using DoodleVideoGeneratorV7_3 (Hierarchy Sorting).
"""
...
from create_doodle_video_v7_3 import DoodleVideoGeneratorV7_3

# --- CONFIGURATION (Same as V7.1/7.2) ---
...
def log(msg):
    print(f"[TopicVideoV7.3] {msg}", flush=True)
...
def generate_beat_manifest(topic):
    log(f"🧠 Generating V7.3 Manifest (with parts) for: {topic}...")
...
def generate_image_fal(prompt, output_path, style="color"):
    ...
    if style == "sketch":
        ...
        style_suffix = ", colorful whiteboard diagram on pure white background, solid marker style, bold solid colors, no shading, no stippling, no texture, bright solid colored boxes and arrows, colored text labels (blue, red, green, orange), thick continuous colorful lines, hand-drawn educational diagram, vibrant solid colors on white, easy to read, natural flow"
        negative_prompt = "grayscale, black and white, monochrome, dull, dark background, plain, boring, tiny text, cramped, 3d, realistic, photo, vertical lines, divider lines, section borders, grid lines, texture, stippling, dot shading, pencil stroke, grainy"
...
def main():
    ...
    parser.add_argument("--output_dir", default="topic_videos_v7_3", help="Output dir")
    ...
    for i, beat in enumerate(beats):
        ...
        # E. Generate Doodle (V7.3)
        if not os.path.exists(final_beat_path):
            try:
                gen = DoodleVideoGeneratorV7_3(
                    image_path, 
                    final_beat_path, 
                    segments=segments,
    ...
    if final_clips:
        log("\n🎞️ Stitching...")
        out_path = os.path.join(work_dir, f"video_v7_3_{safe_topic}.mp4")
