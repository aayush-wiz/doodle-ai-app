...
def generate_beat_manifest(topic):
    ...
    ### CRITICAL VISUAL INSTRUCTIONS: STRICT DOODLE STYLE
    - **Style**: ACCURATE HAND-DRAWN DIAGRAM (Whiteboard Style).
    - **Aesthetic**: Use ACCURATE shapes (Arrays, Trees) but keep the "Hand-Drawn" look.
    - **Color**: Use COLORFUL MARKERS (Blue, Red, Green) for key elements.
    - **NO**: No photorealism, no 3D, no complex shading.
    - **Content**:
      - Describe the visuals as "Hand-drawn whiteboard diagram of..."
    ...
    - **NO FORMULA DICTATION**: Do NOT read formulas aloud (e.g., "Q times K transpose").
      - ❌ BAD: "We compute Q K transpose divided by square root of d k."
      - ✅ GOOD: "We compute the similarity between each word pair and scale it."
    
    ### TARGET OUPUT (JSON ONLY)
    {{
      "topic": "{topic}",
      "beats": [
        {{ "beat_id": 1, "image_prompt": "Simple hand-drawn stick figure sketch of..., Label: '...'", "narrator_script": "..." }},
...
def generate_image_fal(prompt, output_path):
    style_suffix = ", hand-drawn whiteboard diagram, colorful markers, white background, clean bold lines, no shading, high contrast, minimalist"
